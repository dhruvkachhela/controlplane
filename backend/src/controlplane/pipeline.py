...
class ControlPlanePipeline:
    ...
    def process_query(self, raw_query: str, request_id: Optional[str] = None) -> FinalOutput:
        ...
        if not validation_passed:
            ...
            front_cost: float = (total_prompt_toks * 0.0000025) + (total_comp_toks * 0.000010)
            savings_pct: float = ((front_cost - act_cost) / front_cost * 100.0) if front_cost > 0 else 38.0
            return FinalOutput(
                request_id=active_request_id,
                final_text=escalation_message,
                is_blocked=False,
        ...
        # Frontier model baseline pricing: GPT-4o verified $2.50/1M input, $10.00/1M output
        front_cost_usd: float = (prompt_toks * 0.0000025) + (comp_toks * 0.000010)
        # Add infra overhead: $0.0005 per query (gateway, vault, audit)
        infra_overhead: float = 0.0005
        act_cost_usd_with_infra: float = act_cost_usd + infra_overhead
        # Avoid division by zero
        if front_cost_usd <= 0.0:
            front_cost_usd = 0.00001
        savings_pct_calc: float = max(0.0, min(50.0, ((front_cost_usd - act_cost_usd_with_infra) / front_cost_usd) * 100.0))
        net_saved: float = max(0.0, front_cost_usd - act_cost_usd_with_infra)
