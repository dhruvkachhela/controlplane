...
export default function InteractivePlayground() {
  ...
  // Dynamic Active Model State (synced from API / .env)
  const [modelDisplayName, setModelDisplayName] = useState<string>("LAGUNA 2.1 XS");

  // Load saved admin key & fetch active model from API upon mount
  useEffect(() => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setLiveResult({
          ...
          savingsPct: data.savingsPct || 38.0,
        });
      } else {
        setLiveResult(null);
      }
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full font-mono text-xs text-white/80 backdrop-blur-md">
              <span className="w-2 h-2 rounded-full bg-[#FF5500] animate-pulse shrink-0" />
              <span>NVIDIA NIM {modelDisplayName} ONLINE</span>
            </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    {[
                      ...
                      { num: 3, label: `AGENT // ${activeDisplay.modelDisplayName || modelDisplayName}`, badge: activeDisplay.expectedStatus === "PASSED" ? "[ COMPLETED ]" : "[ BYPASS ]", isErr: false },
